@extends('admin.layouts.master')
@section('title', 'Kategori Ekle')
@section('css')
<link rel="stylesheet" href="{{ asset('admin/vendor/select2/css/select2.css') }}" />
<link rel="stylesheet" href="{{ asset('admin/vendor/select2-bootstrap-theme/select2-bootstrap.min.css') }}" />
@endsection
@section('header-title', 'Kategori Ekle')
@section('nav-alt-tab')
<li><span>Kategoriler</span></li>
<li><span>Kategori Ekle</span></li>
@endsection
@section('content')
<section class="card">
    <form action="{{ route('admin.category.store') }}" method="post">
        @csrf
        <header class="card-header">
            <div class="card-actions">
                <a href="javascript:void(0)" class="card-action card-action-toggle" data-card-toggle></a>
            </div>
            <h2 class="card-title">Yeni Kategori</h2>
        </header>
        <div class="card-body">
            <!-- default / right -->
            <div class="row">
                <div class="col">
                    <div class="tabs">
                        <ul class="nav nav-tabs">
                            <li class="nav-item active">
                                <a class="nav-link" data-bs-target="#general" href="#general" data-bs-toggle="tab"><i
                                        class="fas fa-home"></i> Genel</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-target="#seo" href="#seo" data-bs-toggle="tab"><i
                                        class="fas fa-search"></i> SEO</a>
                            </li>
                        </ul>
                        <div class="tab-content">
                            <div id="general" class="tab-pane active">
                                <div class="pb-4 form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="name">Kategori
                                        Adı</label>
                                    <div class="col-lg-6">
                                        <input type="text" class="form-control" name="name" id="name"
                                            placeholder="Elektronik" required>
                                    </div>
                                </div>
                                <div class="pb-4 form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="description">Kategori
                                        Açıklaması</label>
                                    <div class="col-lg-6">
                                        <textarea class="form-control" name="description" id="description"
                                            placeholder="Telefon, Saat, Bilgisayar, Notebook ve Daha fazlası..."></textarea>
                                    </div>
                                </div>
                                <div class="pb-4 form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="parent_id">Üst
                                        Kategori</label>
                                    <div class="col-lg-6">
                                        <select data-plugin-selectTwo class="form-control populate" name="parent_id"
                                            id="parent_id">
                                            <option value="">Yok</option>
                                            @foreach ($categories as $cat)
                                            <option value="{{ $cat->id }}">{{ $cat->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="pb-4 form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="seq">Sıra</label>
                                    <div class="col-lg-6">
                                        <input type="text" class="form-control" name="seq" id="seq" placeholder="0"
                                            required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="status">Durum</label>
                                    <div class="col-lg-6">
                                        <select data-plugin-selectTwo class="form-control populate" name="status"
                                            id="status" data-minimum-results-for-search="Infinity" required>
                                            <option value="0">Pasif</option>
                                            <option value="1" selected>Aktif</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div id="seo" class="tab-pane">
                                <div class="pb-4 form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="meta_title">Meta
                                        Title</label>
                                    <div class="col-lg-6">
                                        <input type="text" class="form-control" name="meta_title" id="meta_title"
                                            placeholder="Elektronik">
                                    </div>
                                </div>
                                <div class="pb-4 form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="meta_desc">Meta
                                        Description</label>
                                    <div class="col-lg-6">
                                        <input type="text" class="form-control" name="meta_desc" id="meta_desc"
                                            placeholder="Telefon, Saat, Bilgisayar, Notebook ve Daha fazlası...">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="pt-2 col-lg-3 control-label text-lg-end" for="meta_keywords">Meta
                                        Keywords</label>
                                    <div class="col-lg-6">
                                        <input type="text" class="form-control" name="meta_keywords" id="meta_keywords"
                                            placeholder="Telefon, Saat, Bilgisayar, Notebook">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <footer class="card-footer">
            <div class="row justify-content-end">
                <div class="col-sm-9">
                    <button type="submit" class="btn btn-primary">Kaydet</button>
                    <button type="reset" class="btn btn-default">Sıfırla</button>
                </div>
            </div>
        </footer>
    </form>
</section>
@endsection
@section('js')
<script src="{{ asset('admin/vendor/select2/js/select2.js') }}"></script>
@endsection
@section('extra-js')
@endsection
